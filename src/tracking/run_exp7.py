...
def run_tracking(tracker_name, yaml_path, conf):
    ...
    for i, l in enumerate(lines):
        if l.startswith("HOTA:"):
            ...
            if i + 1 < len(lines) and lines[i+1].startswith("COMBINED"):
                for h, v in zip(headers, lines[i+1].split()[1:]):
...
if __name__ == "__main__":
    # Step 1: Run combined Experiment 7: conf=0.35 + ORB GMC
    run_tracking("botsort_conf35_orb", "src/tracking/botsort_conf35_orb.yaml", conf=0.35)
    create_eval_zip("botsort_conf35_orb")
    res_comb = evaluate("botsort_conf35_orb")
    
    # Step 2: Test conf=0.40 and conf=0.45
    run_tracking("botsort_conf40", "src/tracking/botsort.yaml", conf=0.40)
    create_eval_zip("botsort_conf40")
    res_40 = evaluate("botsort_conf40")
    
    run_tracking("botsort_conf45", "src/tracking/botsort.yaml", conf=0.45)
    create_eval_zip("botsort_conf45")
    res_45 = evaluate("botsort_conf45")
