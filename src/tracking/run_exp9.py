...
if __name__ == "__main__":
    tracker_reid = "botsort_conf35_orb_match85_reid"
    yaml_path = "src/tracking/botsort_conf35_orb_match85_reid.yaml"
    
    # 1. Run ReID=True
    run_tracking(tracker_reid, yaml_path, conf=0.35)
    create_eval_zip(tracker_reid)
    m_reid = evaluate(tracker_reid)
    
    # 2. Evaluate ReID=False (already run)
    m_no_reid = evaluate("botsort_conf35_orb_match85")
    
    m_no_reid["description"] = "conf=0.35, ORB, match=0.85, ReID=False"
    m_reid["description"] = "conf=0.35, ORB, match=0.85, ReID=True (Exp 9)"
    
    df = pd.DataFrame([m_no_reid, m_reid])
    cols = ["description", "HOTA", "AssA", "IDF1", "IDSW", "Frag", "DetA", "MOTA"]
    print("\n" + "="*80)
    print("EXPERIMENT 9: REID COMPARISON RESULTS:")
    print("="*80)
